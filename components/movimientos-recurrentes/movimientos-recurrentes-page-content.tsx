                ...
                <div key={p.id} className="flex items-center justify-between px-4 py-3 gap-3">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <Button
                      ...
                      className={p.activo ? "text-muted-foreground" : "text-warning"}
                    >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Controller name="monto_estimado" control={control} render={({ field }) => (
              ...
              <div className="space-y-1.5">
                ...
                <Controller name="moneda" control={control} render={({ field }) => (
                  <NamedSelect options={[{ value: "ARS", label: "ARS" }, { value: "USD", label: "USD" }]} value={field.value} onValueChange={v => field.onChange(v ?? "ARS")} />
                )} />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <Controller name="dia_mes" control={control} render={({ field }) => (
                <Input type="number" min={1} max={31} value={field.value ?? ""} onChange={e => field.onChange(e.target.valueAsNumber)} placeholder="1-31" />
              )} />
            ...
            <div className="space-y-1.5">
              ...
              <NamedSelect options={catsPadre.map(c => ({ value: c.id, label: c.nombre }))} value={padreId} onValueChange={v => setPadreId(v)} placeholder="Sin categoría" />
            </div>
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <Controller name="metodo" control={control} render={({ field }) => (
                    <NamedSelect options={METODOS.map(m => ({ value: m, label: m }))} value={field.value ?? null} onValueChange={v => field.onChange(v)} placeholder="Sin especificar" />
                  )} />
                </div>
                <div className="space-y-1.5">
                  <Label>Se debita de</Label>
                  <Controller name="debita_de" control={control} render={({ field }) => (
                    <NamedSelect
                      options={[{ value: "cuenta", label: "Cuenta bancaria" }, { value: "tarjeta", label: "Tarjeta de crédito" }]}
                      value={field.value ?? null} onValueChange={v => { field.onChange(v); }} placeholder="Sin especificar"
                    />
                  )} />
                </div>
                {debita_de === "cuenta" && (
                  <div className="space-y-1.5">
                    ...
                    <Controller name="cuenta_id" control={control} render={({ field }) => (
                      <NamedSelect options={cuentas.map(c => ({ value: c.id, label: c.nombre }))} value={field.value ?? null} onValueChange={v => field.onChange(v)} placeholder="Seleccioná una cuenta" />
                    )} />
                ...
                {debita_de === "tarjeta" && (
                  <div className="space-y-1.5">
                    <Label>Tarjeta</Label>
                    <Controller name="tarjeta_id" control={control} render={({ field }) => (
                      <NamedSelect options={tarjetas.map(t => ({ value: t.id, label: t.nombre }))} value={field.value ?? null} onValueChange={v => field.onChange(v)} placeholder="Seleccioná una tarjeta" />
                    )} />
            ...
            <div className="space-y-1.5">
              ...
              <Controller name="clasificacion" control={control} render={({ field }) => (
                <NamedSelect options={CLASIFICACIONES.map(c => ({ value: c, label: c }))} value={field.value ?? null} onValueChange={v => field.onChange(v)} placeholder="Sin especificar" />
              )} />
            </div>
